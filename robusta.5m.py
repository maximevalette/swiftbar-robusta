...
class RobustaAPI:
    ...
    def fetch_alert_report(self, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.debug:
                print(f"🔍 DEBUG: Report response: {json.dumps(result[:3] if isinstance(result, list) else result, indent=2)}...")
            ...
            if self.debug:
                print(f"DEBUG: API Error fetching report from {self.config.name}: {str(e)}")
                try:
            ...
            if self.debug:
                print(f"DEBUG: Network error fetching report from {self.config.name}: {str(e)}")
            return []
    ...
    def fetch_unresolved_alerts(self, hours_back: int = 24) -> List[Alert]:
        ...
        for idx, report_item in enumerate(report):
            ...
            if self.debug:
                print(f"🔍 DEBUG: Fetching alerts for '{aggregation_key}' ({alert_count} total)")
            ...
            try:
                ...
                if self.debug:
                    print(f"🔍 DEBUG: Response for '{aggregation_key}': {len(alerts_data)} alerts")
                    if alerts_data:
                        print(f"🔍 DEBUG: First alert sample: {json.dumps(alerts_data[0], indent=2)}")
                        ...
                        for ad in alerts_data:
                            ...
                            if ad.get('resolved_at') is None:
                                unresolved_priority_counts[p] = unresolved_priority_counts.get(p, 0) + 1
                            else:
                                resolved_priority_counts[p] = resolved_priority_counts.get(p, 0) + 1
                        print(f"🔍 DEBUG: Priority distribution (all): {priority_counts}")
                        print(f"🔍 DEBUG: Priority distribution (unresolved): {unresolved_priority_counts}")
                        print(f"🔍 DEBUG: Priority distribution (resolved): {resolved_priority_counts}")
                ...
                for alert_data in alerts_data:
                    ...
                    if alert_data.get('resolved_at') is None:
                        ...
                        if self.debug:
                            print(f"🔍 DEBUG: Processing unresolved alert '{alert_data.get('alert_name', 'unknown')}'")
                            print(f"🔍 DEBUG: Raw priority field: {alert_data.get('priority', 'NOT FOUND')}")
                            # Check all possible priority fields
                            for field in ['priority', 'severity', 'level', 'alert_severity', 'alertSeverity']:
                                if field in alert_data:
                                    print(f"🔍 DEBUG: Found field '{field}' = {alert_data[field]}")
                        ...
                        if 'priority' not in alert_data:
                            ...
                            for field in ['severity', 'level', 'alert_severity', 'alertSeverity']:
                                if field in alert_data:
                                    alert_data['priority'] = alert_data[field]
                                    if self.debug:
                                        print(f"🔍 DEBUG: Using '{field}' as priority: {alert_data[field]}")
                                    break
                            else:
                                # Default to LOW if no priority field found
                                alert_data['priority'] = 'LOW'
                                if self.debug:
                                    print(f"🔍 DEBUG: No priority field found, defaulting to LOW")

                        # Normalize priority values to uppercase
                        if 'priority' in alert_data and isinstance(alert_data['priority'], str):
                            original_priority = alert_data['priority']
                            alert_data['priority'] = alert_data['priority'].upper()
                            if self.debug and original_priority != alert_data['priority']:
                                print(f"🔍 DEBUG: Normalized priority from '{original_priority}' to '{alert_data['priority']}'")
                        ...
                        try:
                            ...
                            if self.debug:
                                print(f"🔍 DEBUG: Alert created with final priority: {alert.priority}")
                            ...
                            if self.config.dashboard_url:
                                ...
                                url += f"?dates=21600"
                                url += f"&grouping=%22ALERT_NAME%22"
                                url += f"&events=%5B{alert_name_encoded}%5D"
                                alert._robusta_url = url
                            all_alerts.append(alert)
                        except Exception as e:
                            if self.debug:
                                print(f"🔍 DEBUG: Failed to create alert. Data keys: {list(alert_data.keys())}")
                                print(f"🔍 DEBUG: Error: {str(e)}")
                            print(f"Warning: Could not parse alert data for {aggregation_key}: {str(e)}")
                            continue
...
class SwiftBarRenderer:
        ...
        for idx, cluster_name in enumerate(cluster_names):
            ...
            for priority in priority_order:
                if priority in alerts_by_priority:
                    ...
                    print(f"{symbol} {priority} ({len(priority_alerts)}) | color={color}")
    ...
    def _render_alert_item(self, alert: Alert):
        ...
        if self.config.show_namespace:
            parts.append(f"{self._sanitize_for_menu(str(alert.namespace))}/{self._sanitize_for_menu(alert.resource_name)}")
        else:
        ...
        print(f"---- Node: {self._sanitize_for_menu(str(alert.resource_node))} | color=#898989")
        ...
        print(f'---- Copy Alert Details | bash="echo {encoded_text} | base64 -d | pbcopy" terminal=false')
    ...
    def _render_footer(self):
        ...
        print("Open config | href=file://" + str(Path("~/.config/swiftbar/robusta.yml").expanduser()))
...
def detect_changes(current_alerts: List[Alert], previous_state: Dict[str, Any]) -> Tuple[List[Alert], List[Dict[str, Any]]]:
...
def load_config() -> tuple[List[ClusterConfig], DisplayConfig]:
    ...
    config_path = Path(os.environ.get('VAR_CONFIG_PATH', '~/.config/swiftbar/robusta.yml')).expanduser()
    ...
    try:
        ...
        config_data['display'] = {**default_config['display'], **config_data.get('display', {})}
...
def main():
    ...
    try:
        ...
        for cluster_config in clusters_config:
            ...
            alerts = api.fetch_unresolved_alerts(hours_back=display_config.stale_alert_hours)
        ...
        if new_alerts:
            ...
            critical_high = new_by_priority.get('CRITICAL', []) + new_by_priority.get('HIGH', [])
            if critical_high:
                alert_names = [f"{a.alert_name} ({a.cluster})" for a in critical_high[:3]]  # Show up to 3
